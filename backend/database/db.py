...
Uses sqlite3 with WAL mode, row factory, and thread-safe connection
management. No external database dependencies — works entirely with
a local file at DB_PATH.
...
def get_connection():
    """Get or create the SQLite database connection. Thread-safe with lock."""
    global _conn
...
# ─── Helper: execute with SQLite-only placeholder ──────────

def _execute(conn, sql: str, params: tuple = None):
    """Execute SQL with optional params. SQLite uses ? placeholders."""
...
def config_get(key: str, default: str = "") -> str:
    conn = get_connection()
    cursor = _execute(conn, "SELECT value FROM config WHERE key = ?", (key,))
    row = cursor.fetchone()
    if row:
        return row[0]
    return default
...
def trade_close(trade_id: int, exit_price: float, exit_time: str,
    ...
    now_expr = _now_sql()
    _execute(conn,
        f"UPDATE trades SET status = 'closed', exit_price = ?, exit_time = ?, "
        f"pnl_usdt = ?, pnl_pct = ?, exit_reason = ?, "
...
def get_last_processed_candle(coin: str) -> Optional[str]:
    ...
    conn = get_connection()
    cursor = _execute(conn,
        "SELECT timestamp FROM processed_candles WHERE coin = ? "
